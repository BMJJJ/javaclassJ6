...
//회원 전체/부분 리스트
	public ArrayList<MemberVO> getMemberList(int startIndexNo, int pageSize, int level) {
		...
		try {
			if(level == 999) {
				sql = "select *, timestampdiff(day, lastDate, now()) as deleteDiff from member2 order by idx desc limit ?,?";
				pstmt = conn.prepareStatement(sql);
				pstmt.setInt(1, startIndexNo);
				pstmt.setInt(2, pageSize);
			}
			else {
				sql = "select *, timestampdiff(day, lastDate, now()) as deleteDiff  from member2 where level = ? order by idx desc limit ?,?";
				pstmt = conn.prepareStatement(sql);
...
//회원 등급 변경처리
	public int setMemberLevelChange(int idx, int level) {
		...
		try {
			if(level == 99) {
				sql = "update member2 set level = ?, lastDate=now(), userDel='OK' where idx = ?";
			}
			else {
				sql = "update member2 set level = ?, userDel='NO' where idx = ?";				
			}
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, level);
			pstmt.setInt(2, idx);
...
//각 레벨별 건수 구하기
	public int getTotRecCnt(int level) {
		...
		try {
			if(level == 999) {
				sql = "select count(*) as cnt from member2";
				pstmt = conn.prepareStatement(sql);
			}
			else {
				sql = "select count(*) as cnt  from member2 where level = ? order by idx desc";
				pstmt = conn.prepareStatement(sql);
				pstmt.setInt(1, level);
			}
			rs = pstmt.executeQuery();
